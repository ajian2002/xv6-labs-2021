...
found:
    ...
    if ((p->shared_pagetable = (struct usyscall *)kalloc()) == 0)
    {
        freeproc(p);
        release(&p->lock);
        return 0;
    }
    p->shared_pagetable->pid = p->pid;
...
static void freeproc(struct proc *p)
{
    ...
    p->trapframe = 0;
    if (p->shared_pagetable) kfree((void *)p->shared_pagetable);
    p->shared_pagetable = 0;
...
pagetable_t proc_pagetable(struct proc *p)
{
    ...
    // map
    if (mappages(pagetable, USYSCALL, PGSIZE, (uint64)((p->shared_pagetable)), PTE_R | PTE_U) < 0)
    {
        uvmunmap(pagetable, TRAMPOLINE, 1, 0);
